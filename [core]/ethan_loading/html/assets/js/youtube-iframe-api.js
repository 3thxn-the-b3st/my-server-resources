/*
 * Local YouTube IFrame API (self-contained).
 *
 * Replaces <script src="https://www.youtube.com/iframe_api">.
 * The upstream URL is a ~1KB loader that injects a versioned www-widgetapi.js
 * from Google's CDN. Every browser/DUI context that loaded it made its own
 * request to YouTube; with many audio/TV DUIs alive at once that produced a
 * burst of external fetches and Permissions-Policy header spam, which is
 * implicated in CEF (CrBrowserMain) crashes.
 *
 * Here the loader's bootstrap and the widget API payload are concatenated, so
 * YT.Player is available with zero network access.
 *
 * Pinned player build: ea6f527e
 * Vendored: 2026-08-05
 * To refresh: fetch https://www.youtube.com/iframe_api, read the scriptUrl it
 * points at, and re-concatenate that file below this header.
 *
 * NOTE: playing a video still contacts YouTube - only API delivery is local.
 */
var YT; if (!window["YT"]) YT = { loading: 0, loaded: 0 };
var YTConfig; if (!window["YTConfig"]) YTConfig = { "host": "https://www.youtube.com" };
if (!YT.loading) {
  YT.loading = 1;
  (function () {
    var l = [];
