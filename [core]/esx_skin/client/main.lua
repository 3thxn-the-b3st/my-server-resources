...
RegisterNetEvent("esx_skin:openSaveableMenu", function(submitCb, cancelCb)
    -- Wrap the submit callback so job_quiz triggers right after saving
    local wrappedSubmit = function(data, menu)
        if submitCb then
            submitCb(data, menu)
        end
        TriggerEvent('job_quiz:openMenu')
    end

    Menu:Saveable(wrappedSubmit, cancelCb, nil)
end)
